...
if 'GBM_grid_search_' + metric + '.json' not in os.listdir('../Datasets'):
	outcomes = {}
	
else:
	with open('GBM_grid_search_' + metric + '.json') as json_file:
...
#####################################################CV DATA MODELING###############################################################
	...
	try:
		...
		kfolds = KfoldsCV(method='GBM', num_folds=3, grid_param={'subsample': [0.75],
			                                                     'learning_rate': [0.05],
			                                                     'max_depth': [1,2,3,4,5],
			                                                     'n_estimators': [500]},
...
######################################################ANALYSIS OF RESULTS###########################################################
	...
	if export == 'yes':
		with open('GBM_grid_search_' + metric + '.json', 'w') as json_file:
